...
def getsensitivity(similaritymatrix):
    print('ruyyi')
    return 0

def createnoisematrix(similarity_matrix,sensitivity):
    noise_matrix = np.random.normal(loc = 0, scale = sensitivity, size = similarity_matrix.shape)
    return noise_matrix

def addnoisematrix(similarity_matrix,sensitivity):
    noise_matrix = createnoisematrix(similarity_matrix,sensitivity)
...
def initiaterecommendation(testset):
    ...
    return k,result,userbased_pred,itembased_pred,fuse_pred
...
if __name__ == "__main__" :
    ...
    trainset,testset = splitdataset(0.2,recordsize,ratings)
    ...
    k, result, userbased_pred, itembased_pred, fuse_pred = initiaterecommendation(testset)
    for _, row in result.iterrows():
        ...
        if userneighbor.shape[0] != 0:
            userbased_pred.append(userbasedrecommendation(userneighbor))
        else:userbased_pred.append(3)
        if movieneighbor.shape[0] != 0:
            itembased_pred.append(itembasedrecommendation(movieneighbor))
        else:itembased_pred.append(3)
